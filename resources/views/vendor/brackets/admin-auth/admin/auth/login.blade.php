@extends('layouts.backend.master-black')
                    ...
                    <div class="card-block">
                        <auth-form
                            ...
                            <form class="form-horizontal" role="form" method="POST" action="{{ url('/admin/login') }}" novalidate>
                                {{ csrf_field() }}
                                <div class="auth-header" style="background: #181818; border: none;">
                                    <div style="text-align: center; margin-bottom: 30px"><img src="@include('logos.logo-full')" alt="{{ env('APP_NAME') }}" style="max-width: 400px; max-height: 40px;"></div>
                                    <h1 class="auth-title" style="color: #E1454F">{{ trans('brackets/admin-auth::admin.login.title') }}</h1>
                                    <p class="auth-subtitle">{{ trans('brackets/admin-auth::admin.login.sign_in_text') }}</p>
                                </div>
                                <div class="auth-body" style="background: #181818">
