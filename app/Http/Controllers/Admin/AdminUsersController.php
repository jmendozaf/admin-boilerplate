...
class AdminUsersController extends Controller
{
    ...
    public function create()
    {
        ...
        $WhatToDos = WhatToDo::all();
        $WhatToEats = WhatToEat::all();
        $WhatToSees = WhatToSee::all();

        return view('admin.admin-user.create', [
            'activation' => Config::get('admin-auth.activation_enabled'),
            'roles' => Role::where('guard_name', $this->guard)->get(),
            'WhatToDos' => $WhatToDos,
            'WhatToEats' => $WhatToEats,
            'WhatToSees' => $WhatToSees,
        ]);
    }
    ...
    public function store(StoreAdminUser $request)
    {
        ...
        $adminUser->whattodos()->sync(collect($request->input('whattodos', []))->map->id->toArray());
        $adminUser->whattoeats()->sync(collect($request->input('whattoeats', []))->map->id->toArray());
        $adminUser->whattosees()->sync(collect($request->input('whattosees', []))->map->id->toArray());
    ...
    public function edit(AdminUser $adminUser)
    {
        ...
        $adminUser->load('whattodos');
        $adminUser->load('whattoeats');
        $adminUser->load('whattosees');
    ...
    public function update(UpdateAdminUser $request, AdminUser $adminUser)
    {
        ...
        if ($request->input('whattodos')) {
            $adminUser->whattodos()->sync(collect($request->input('whattodos', []))->map->id->toArray());
        }else{
            $adminUser->whattodos()->sync(collect($request->input('whattodos', []))->map->id->toArray());
        }

        if ($request->input('whattoeats')) {
            $adminUser->whattoeats()->sync(collect($request->input('whattoeats', []))->map->id->toArray());
        }else{
            $adminUser->whattoeats()->sync(collect($request->input('whattoeats', []))->map->id->toArray());
        }

        if ($request->input('WhatToSees')) {
            $adminUser->WhatToSees()->sync(collect($request->input('whattosees', []))->map->id->toArray());
        }else{
            $adminUser->whattosees()->sync(collect($request->input('whattosees', []))->map->id->toArray());
        }
